...
class Oauth {
    ...
    /**
     * Laravel socialite manger instance.
     *
    ...
    /**
     * EnvatoApiClient service instance.
     *
     * @var EnvatoApiClient
     */
    private $envato;
     ...
     */
    public function __construct(SocialiteFactory $socialite, User $user, UserRepository $userCreator, Settings $settings, EnvatoApiClient $envato, PurchaseCode $purchaseCode)
    {
        $this->user = $user;
        $this->envato = $envato;
    ...
    {
        if (Auth::user()) {
           return View::make('oauth/popup')->with('status', 'ALREADY_LOGGED_IN');
        }
    ...
    public function createUserFromOAuthData($data, $returnView = false)
    {
        ...
        if ( ! $user) {
            $user = $this->userCreator->create(['email' => $profile->email, 'avatar' => $profile->avatar]);
        }
    ...
    public function attachProfileToExistingUser($user, $profile, $service)
    {
        ...
        return $this->getPopupResponse('SUCCESS_CONNECTED', ['user' => $user->load('social_profiles', 'purchase_codes', 'groups')]);
    }
    ...
    {
        $user = Auth::loginUsingId($user->id, true)->load('groups')->toArray();
    ...
    {
        return response()->view('oauth.popup', ['status' => 'ERROR', 'data' => $message], 500);
    }
    ...
    {
        $view = View::make('oauth/popup')->with('status', $status);
